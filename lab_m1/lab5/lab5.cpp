...
void Lab5::Init()
{
...

    {
        ...
        mesh->LoadMesh(PATH_JOIN(window->props.selfDir, RESOURCE_PATH::MODELS, "tema2"), "Tun1.obj");
        meshes[mesh->GetMeshID()] = mesh;
    }
...
void Lab5::Update(float deltaTimeSeconds)
{
...

    {
        ...
        RenderMesh(meshes["senile"], shaders["Simple"], modelMatrix);
    }

    {
        glm::mat4 modelMatrix = glm::mat4(1);
        modelMatrix = glm::translate(modelMatrix, glm::vec3(5, 0.5f, 0));
        RenderMesh(meshes["tun"], shaders["Simple"], modelMatrix);
    }
    ...
    // implemented, and the local projection matrix.
    {
        ...
        modelMatrix = glm::rotate(modelMatrix, RADIANS(105.0f), glm::vec3(1, 0, 0));
...
void Lab5::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
    {
        ...
        float distance = cameraSpeed * 0.1f;
    ...
    if (window->KeyHold(GLFW_KEY_G)) {
        raza += 5;
        fov = RADIANS(raza);
        projectionMatrix = glm::perspective<float>(fov, aspect, zNear, zFar);
    }
    ...
    if (window->KeyHold(GLFW_KEY_1)) {
        top += 0.1;
        bottom -= 0.1;
        projectionMatrix = glm::orthoLH_ZO<float>(left, right, bottom, top, 100, 250);
    }
...
void Lab5::OnMouseMove(int mouseX, int mouseY, int deltaX, int deltaY)
{
    ...
    if (window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
    {
        ...
        if (window->GetSpecialKeyState() == 0) {
           ...
           // camera->RotateFirstPerson_OZ(deltaX * sensivityOY);
        }
