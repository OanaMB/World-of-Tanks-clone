...
void Lab8::Init()
{
    ...
    {
        //lightPosition = {glm::vec3(0, 1, 1), glm::vec3(1,1,1)};
...
{
    {
        ...
        RenderSimpleMesh(meshes["sphere"], shaders["LabShader"], modelMatrix, glm::vec3(0.65, 0.75, 0.45));
...

    {
        ...
        // TODO(student): Add or change the object colors
        RenderSimpleMesh(meshes["box"], shaders["LabShader"], modelMatrix, glm::vec3(0.35, 0.65, 0.85));
...
void Lab8::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
    ...
    glUniform1iv(glGetUniformLocation(shader->program, "spotlight"), 2, spotlight);
    glUniform1fv(glGetUniformLocation(shader->program, "cut_off_angle"), 2, cutoffAngle);
...
void Lab8::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (!window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
    {
        ...
        if (window->KeyHold(GLFW_KEY_0)) lightPosition[1] += up * deltaTime * speed;
        if (window->KeyHold(GLFW_KEY_9)) lightPosition[1] -= up * deltaTime * speed;
        ...
        {
            cutoffAngle[0] += deltaTime * ANGLE_SPEEDUP;
        }
        ...
        turn = glm::rotate(turn, angleZ, glm::vec3(0, 0, 1));
